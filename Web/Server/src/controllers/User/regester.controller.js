...
const InActiveRegister = asynHandler(async (req, res) => {
  ...
  const existedemalUser = await inactiveusers.findOne({ email });
  const existednameUser = await inactiveusers.findOne({ username });
  if (existedemalUser) {
  ...
  const createdUser = await inactiveusers.create(newUser);
